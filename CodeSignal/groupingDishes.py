# SOLUTION 1 - Passed 7/8 test cases
def groupingDishes(dishes):
    ...
    dup_ings = [*ings[:]]
    for ing in sorted(list(set(ings)),  key=str.lower):
        if dup_ings.count(ing) > 1:
            ing_dishes = sorted([dish[0] for dish in dishes if ing in dish],  key=str.lower)
            results.append([ing, *ing_dishes])
    return results
